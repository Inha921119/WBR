...
<%@ include file="../common/toastUiEditorLib.jsp" %>
	...
	<section class="mt-8 text-xl h-screen">
		<div class="container mx-auto px-3 pb-5 border-bottom-line text-black">
			...
			<div class="btns flex justify-between items-end">
				<div class="mt-2">
					<button class="btn btn-active" type="button" onclick="history.back();">뒤로가기</button>
					<c:if test="${article.actorCanChangeData }">
						<a class="btn btn-active" href="modify?id=${article.id }">수정</a>
						<a class="btn btn-active" href="doDelete?id=${article.id }" onclick="if(confirm('정말 삭제하시겠습니까?') == false) return false;">삭제</a>
					</c:if>
				</div>
				<div class="mt-2">
					<c:if test="${rq.getLoginedMemberId() != 0 }">
						<a id="goodBtn" class="btn btn-active" href="../reactionPoint/doInsertReactionPoint?relId=${article.id }&relTypeCode=article&point=1">👍 ${article.goodReactionPoint }</a>
						<a id="badBtn" class="btn btn-active" href="../reactionPoint/doInsertReactionPoint?relId=${article.id }&relTypeCode=article&point=-1">👎 ${article.badReactionPoint * -1 }</a>
					</c:if>
				</div>
			</div>
		</div>
			 	...
			 	let addHtml = `
			 		<form action="../reply/doModify" method="POST" onsubmit="replyWrite_submitForm(this); return false;">
						...
						<div class="mt-2 border border-gray-400 rounded-lg text-base p-4">
							...
							<a class="btn btn-active btn-sm mr-2" onclick="replyModify_cancle(\${i});">취소</a>
							<button class="btn btn-active btn-sm">등록</button></div>
						</div>
					</form>`;
	...
	<section class="mt-5 text-xl h-screen">
			...
			<c:if test="${rq.getLoginedMemberId() != 0 }">
				<form action="../reply/doWrite" method="POST" onsubmit="replyWrite_submitForm(this); return false;">
					...
					<div class="mt-4 border border-gray-400 rounded-lg text-base p-4">
						...
						<div class="flex justify-end"><button class="btn btn-active btn-sm">등록</button></div>
					</div>
				</form>
			</c:if>
		</div>
